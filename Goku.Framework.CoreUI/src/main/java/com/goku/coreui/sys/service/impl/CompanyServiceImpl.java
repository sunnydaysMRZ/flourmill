...
import com.goku.coreui.sys.mapper.FlourmillCompanyMapper;
...
public class CompanyServiceImpl implements CompanyService {
    ...
    @Autowired
    FlourmillCompanyMapper flourmillCompanyMapper;

    @Override
    public FlourmillCompany selectByPrimaryKey(Integer companyId) {
        return flourmillCompanyMapper.selectByPrimaryKey(companyId);
    }

	@Override
	public int saveOrEditCompany(Integer companyid , String companyName) {
		FlourmillCompany flourmillCompany=new FlourmillCompany(); 
		if(companyid==null){
			flourmillCompany.setIdDel(0);
			flourmillCompany.setCompanyName(companyName);
			flourmillCompany.setCreateTime(DateUtil.nowDate());
			return flourmillCompanyMapper.insert(flourmillCompany);
		}else{
			flourmillCompany=flourmillCompanyMapper.selectByPrimaryKey(companyid);
			flourmillCompany.setCompanyName(companyName);
			return flourmillCompanyMapper.updateByPrimaryKeySelective(flourmillCompany);
		}
	}

	 @Override
	 public PageInfo getCompanyForPaging(String username, String orderFiled, String orderSort, Integer pageindex, Integer pagenum) {
	        PageHelper.startPage(pageindex, pagenum);
	        List<FlourmillCompany> list = flourmillCompanyMapper.getCompanyForPaging(username,orderFiled, orderSort);
